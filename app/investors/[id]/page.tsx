...
export default async function InvestorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 md:px-8"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-5"><h2 className="text-lg font-semibold">关联基金／申报主体与归属</h2>{funds.length ? <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{funds.map((fund) => <a key={fund.id} href={fund.sourceUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-slate-800 bg-slate-950 p-3 hover:border-cyan-700"><h3 className="text-sm font-semibold text-violet-200">{fund.id}</h3><p className="mt-1 text-xs text-slate-300">{fund.name}</p><p className="mt-2 text-[11px] text-slate-500">{fund.relationship}</p><p className="mt-2 text-[11px] text-cyan-300">{fund.coverageNote ?? (fund.holdingsUrl ? '持仓快照 + 官方交易通知' : '仅交易通知；持仓暂未接入')}</p></a>)}</div> : <p className="mt-3 text-sm text-slate-400">持仓／交易源尚未接入，不据此推断该主体无持仓。</p>}</section>
